 ...
 *
 * Multipart export to File Cabinet (keeps each file <10MB SuiteScript limit):
 * - fulfillments_0001.jsonl, fulfillments_0002.jsonl, ...
 * - fulfillment_lines_0001.jsonl, fulfillment_lines_0002.jsonl, ...
...
define(["N/query", "N/file", "N/search", "N/runtime", "N/log"], function (
  ...
  function summarize(summary) {
    ...
    // Create directly as final name (no file.load rename)
    function createTempThenRename(finalName, contents, fileType) {
      ...
      // extra safety: delete duplicates with same name
    ...
    // linesArr: array of strings, each ends with "\n"
    // returns parts: [{id,name,rows}]
    ...
    // Write parts
    var fulfillParts = writeJsonlParts("fulfillments", fulfillmentOut);
    var lineParts = writeJsonlParts("fulfillment_lines", lineOut);
    ...
    var manifest = {
      ...
      notes: {
        ...
        tracking_numbers_supported: true,
      },
      files: {
        fulfillments: {
          total_rows: fulfillmentCount,
          parts: fulfillParts, // [{id,name,rows}]
        },
        fulfillment_lines: {
          total_rows: lineCount,
          parts: lineParts, // [{id,name,rows}]
        },
      },
    };
    ...
    log.audit("Fulfillment export complete (multipart)", {
      folderId: folderId,
      fulfillments: fulfillmentCount,
      lines: lineCount,
      fulfillParts: fulfillParts.length,
      lineParts: lineParts.length,
    });
  }
