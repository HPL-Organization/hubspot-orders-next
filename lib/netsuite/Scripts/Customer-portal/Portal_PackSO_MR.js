...
define(["N/query", "N/file", "N/search", "N/runtime", "N/log"], function (
  ...
  function summarize(summary) {
    ...
    var targetSeenInDeposits = false;
    ...
    for (;;) {
      ...
      var depositsQ =
        "SELECT DISTINCT " +
        "  NTL.previousdoc AS soId, " +
        "  CD.id AS depositId, " +
        "  CD.tranid AS tranId, " +
        "  CD.trandate AS trandate, " +
        "  CD.entity AS customerId, " +
        "  CD.foreigntotal AS amountFx, " +
        "  CD.memo AS memo, " +
        "  BUILTIN.DF(CD.status) AS status " +
        "FROM NextTransactionLink NTL " +
        "JOIN transaction SO ON SO.id = NTL.previousdoc " +
        "JOIN transaction CD ON CD.id = NTL.nextdoc " +
        "WHERE SO.type = 'SalesOrd' " +
        "  AND CD.type = 'CustDep' " +
        "  AND NTL.previousdoc IN (" +
        csv +
        ")";
      ...
      var dp = query.runSuiteQL({ query: depositsQ }).asMappedResults() || [];
      ...
      for (var c = 0; c < dp.length; c++) {
        if (Number(dp[c].soid) === TARGET_SO) {
          targetSeenInDeposits = true;
          log.audit("TARGET SO FOUND IN DEPOSITS QUERY", dp[c]);
        }
      }
      ...
      for (var m = 0; m < dp.length; m++) {
        ...
        depositLines.push(
          JSON.stringify({
            deposit_id: dep.depositid != null ? Number(dep.depositid) : null,
            tran_id: dep.tranid || null,
            trandate: dep.trandate || null,
            customer_id: dep.customerid != null ? Number(dep.customerid) : null,
            so_id: dep.soid != null ? Number(dep.soid) : null,
            amount: Number(dep.amountfx || 0),
    ...
    log.audit("TARGET SO SUMMARY", {
      ...
      seenInDeposits: targetSeenInDeposits,
    });
    ...
    var depositParts = writeJsonlParts(
