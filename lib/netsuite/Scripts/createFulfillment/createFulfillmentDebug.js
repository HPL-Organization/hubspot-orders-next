 ...
 */
define(["N/query", "N/record", "N/log", "N/email", "N/runtime"], function (
  query,
  record,
  log,
  email,
  ...
  var NOTIFY_EMAIL = "raktim@hplapidary.com";
  var DEBUG_DRY_RUN = true;
  ...
  function getAddressSnapshot(parentRec, subrecordFieldId) {
    var out = {
      exists: false,
      values: {
        addr1: "",
  ...
  function reduce(context) {
    ...
    try {
      ...
      log.error("Load SO failed", {
        salesorder: soIdNum,
        tranid: tranId,
        err: eLoad,
      });
      context.write("__error__", String(soIdNum));
      writeNotFulfilled(context, soIdNum, tranId, "Load SO failed", {
        error: String(eLoad),
      });
      return;
    }

    var shippingAddress = getAddressSnapshot(soRec, "shippingaddress");
    var billingAddress = getAddressSnapshot(soRec, "billingaddress");
    ...
    if (billingAddress.error) {
      warnings.push(
        "Could not fully inspect billing address: " +
          String(billingAddress.error),
      );
      log.error("Billing address read error", {
        salesorder: soIdNum,
        tranid: tranId,
        err: billingAddress.error,
      });
    }
    ...
    if (!billingAddress.exists) {
      warnings.push("Billing address subrecord missing");
    } else {
      var billingMissing = [];
      var billingFields = ["addr1", "addr2", "city", "state", "zip"];
      for (var bf = 0; bf < billingFields.length; bf++) {
        if (isBlank(billingAddress.values[billingFields[bf]])) {
          billingMissing.push(billingFields[bf]);
        }
      }
      if (billingMissing.length) {
        warnings.push(
          "Billing address missing fields: " + billingMissing.join(", "),
        );
      }
    }
    ...
    if (DEBUG_DRY_RUN) {
      ...
      log.audit("DEBUG dry run - fulfillment not created", {
        salesorder: soIdNum,
        tranid: tranId,
        linesSelected: selected,
      });
  ...
  function summarize(summary) {
    ...
    summary.output.iterator().each(function (key, value) {
      ...
      } else if (key === "__debug_would_fulfill__") {
        try {
          debugWouldFulfillRows.push(JSON.parse(String(value)));
        } catch (_) {}
      }
    ...
    log.audit("Run totals", {
      ...
      wouldCreateFulfillments: debugWouldFulfillRows.length,
    });
    ...
    if (debugWouldFulfillRows.length) {
      log.audit("DEBUG dry run - would create fulfillments", {
        count: debugWouldFulfillRows.length,
        rows: debugWouldFulfillRows,
      });
    }

    if (
      notFulfilledRows.length ||
      debugWouldFulfillRows.length ||
      warningRows.length
    ) {
      try {
        var subject = "Auto-fulfillment debug run summary";
        var body = [];
        body.push("Auto-fulfillment ran in DEBUG DRY RUN mode.");
        body.push("No item fulfillments were actually created.");
        body.push("");
        body.push("Summary:");
        body.push("Processed SOs: " + soCount);
        body.push("Qualified Lines: " + totalQualifiedLines);
        body.push("Would Create Fulfillments: " + debugWouldFulfillRows.length);
        ...
        if (debugWouldFulfillRows.length) {
          body.push("Would-have-created fulfillments:");
          for (var d = 0; d < debugWouldFulfillRows.length; d++) {
            var dr = debugWouldFulfillRows[d] || {};
            body.push(
              [
                "SO ID: " + safeString(dr.soId),
                "Tran ID: " + safeString(dr.tranId),
                "Lines Selected: " + safeString(dr.linesSelected),
              ].join(" | "),
            );
          }
          body.push("");
        ...
        if (notFulfilledRows.length) {
          ...
          body.push("");
          for (var i = 0; i < notFulfilledRows.length; i++) {
            var row = notFulfilledRows[i] || {};
            body.push(
              [
                "SO ID: " + safeString(row.soId),
                "Tran ID: " + safeString(row.tranId),
        ...
        log.audit("Debug summary email sent", {
        ...
        log.error("Failed to send debug summary email", {
          recipient: NOTIFY_EMAIL,
          err: eEmail,
        });
      }
