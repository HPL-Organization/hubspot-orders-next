...
define([
  ...
  function onRequest(ctx) {
    ...
    if (warnList.length && !confirmed) {
      ...
      f.addField({
        ...
        <div style="font-family:system-ui,Arial,sans-serif">
          <style>
            .hpl-btn{display:inline-flex;align-items:center;gap:8px;padding:10px 14px;border-radius:10px;text-decoration:none;font-weight:600;border:1px solid transparent}
            .hpl-btn--primary{background:#0ea5e9;color:#fff;border-color:#0284c7;box-shadow:0 6px 18px rgba(2,132,199,.25)}
            .hpl-btn--primary:hover{background:#0284c7}
            .hpl-btn--ghost{background:#fff;color:#111827;border-color:#d1d5db}
            .hpl-btn--ghost:hover{background:#f9fafb}
            .hpl-actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:12px}
            .hpl-fab{position:fixed;right:16px;bottom:16px;z-index:9999}
          </style>
          ...
          <div class="hpl-actions">
            <a href="${proceedUrl}" class="hpl-btn hpl-btn--primary">Proceed anyway</a>
            <a href="${soUrl}" class="hpl-btn hpl-btn--primary">Back to Sales Order</a>
          </div>

          <div class="hpl-fab">
            <a href="${soUrl}" class="hpl-btn hpl-btn--primary" aria-label="Back to Sales Order">Back to Sales Order</a>
          </div>
        </div>
      `;
      ctx.response.writePage(f);
    ...
    form.addField({
      ...
      <div style="font-family:system-ui,Arial,sans-serif">
        <style>
          .hpl-toolbar{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:10px}
          .hpl-btn{display:inline-flex;align-items:center;gap:8px;padding:10px 14px;border-radius:10px;text-decoration:none;font-weight:600;border:1px solid transparent}
          .hpl-btn--primary{background:#0ea5e9;color:#fff;border-color:#0284c7;box-shadow:0 6px 18px rgba(2,132,199,.25)}
          .hpl-btn--primary:hover{background:#0284c7}
          .hpl-btn--ghost{background:#fff;color:#111827;border-color:#d1d5db}
          .hpl-btn--ghost:hover{background:#f9fafb}
          .hpl-fab{position:fixed;right:16px;bottom:16px;z-index:9999}
          .hpl-subtle{color:#4b5563}
        </style>

        <div class="hpl-toolbar">
          <h2 style="margin:0">Unwind Related — Financials</h2>
          <a href="${soUrl}" class="hpl-btn hpl-btn--primary" aria-label="Back to Sales Order">Back to Sales Order</a>
        </div>

        <div>Sales Order: <b>${h(soId)}</b></div>
        <div class="hpl-subtle" style="margin:8px 0">Unapplied payments & deposit applications removed; invoices deleted when fully clear.</div>
        ...
        <div style="margin-top:12px">
          <a href="${soUrl}" class="hpl-btn hpl-btn--ghost">Back to Sales Order</a>
        </div>
