...
define(["N/ui/serverWidget", "N/query", "N/url", "N/log"], function (
  ...
  function onRequest(ctx) {
    ...
    // FIX: these must allow decimals, because NetSuite quantities can be fractional (e.g., 2.01)
    sub.addField({
      id: "col_qty",
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      sub.setSublistValue({
        id: "col_qty",
        line: i,
        value: toNumStr(r.qty),
      });
      sub.setSublistValue({
        id: "col_qty_committed",
        line: i,
        value: toNumStr(r.qty_committed),
      });
      sub.setSublistValue({
        id: "col_qty_backordered",
        line: i,
        value: toNumStr(r.qty_backordered),
      });
