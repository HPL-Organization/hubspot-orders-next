...
define(["N/ui/serverWidget", "N/search", "N/runtime"], function (
  ...
  function runGlobal() {
    ...
    paged.pageRanges.forEach(function (rg) {
      ...
      pg.data.forEach(function (r) {
        ...
        var memberId = r.getValue({ name: "memberitem" });
        var memberQty = n(r.getValue({ name: "memberquantity" }));
        if (!memberId || memberQty <= 0) return;
        var avail = n(
  ...
  function renderHTML(ctx, groups) {
    ...
    groups.forEach(function (g) {
      ...
      html.push("<table>");
      html.push(
        '<thead><tr><th style="width:56%">Member Item</th><th class="num" style="width:16%">Per Kit Qty</th><th class="num" style="width:28%">Availability</th></tr></thead>'
      );
      html.push('<tbody id="t' + gid + '" style="display:none">');
      g.rows.forEach(function (r) {
        html.push("<tr>");
        html.push("<td>" + String(r.memberId) + "</td>");
