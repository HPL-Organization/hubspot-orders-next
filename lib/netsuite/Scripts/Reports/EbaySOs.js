  ...
  function runReport() {
    const soSearch = search.create({
      ...
      filters: [
        ...
        "AND",
        [
          "formulanumeric: CASE WHEN UPPER({salesrep}) = 'EBAY' OR UPPER({cseg_nsps_so_class}) = 'MARKETPLACE : EBAY' THEN 1 ELSE 0 END",
          "equalto",
          "1",
        ],
      ],
    ...
    log.debug({ title: "eBay SO count", details: results.length });
    return results;
  }
  ...
  function onRequest(context) {
    ...
    if (context.request.parameters.csv === "T") {
      ...
      context.response.addHeader({
        ...
        value: 'attachment; filename="ebay_sales_orders.csv"',
      });
