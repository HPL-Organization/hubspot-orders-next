...
define([
  ...
  function parsePaidAtToDate(raw) {
  ...
  function formatAgeFromPaidAt(raw) {
    var d = parsePaidAtToDate(raw);
  ...
  function parseHoldTillToDate(raw) {
    if (raw === null || raw === undefined) return null;
    var s = String(raw).trim();
    if (!s) return null;
  ...
  function isFutureHoldTill(raw) {
    var d = parseHoldTillToDate(raw);
    if (!d) return false;
  ...
  function splitRowsByHoldTill(rows) {
    var visible = [];
    var skipped = [];

    for (var i = 0; i < rows.length; i++) {
      var r = rows[i];
      if (isFutureHoldTill(r.hold_till)) skipped.push(r);
      else visible.push(r);
    }

    return {
      visibleRows: visible,
      skippedHoldTillRows: skipped,
    };
  }
    ...
    var sql =
      ...
      WITH base AS (\
        SELECT\
          ...
          l.custcol_hpl_line_paid_at             AS line_paid_at\
        FROM transaction o\
        ...
        SELECT\
          so_id, so_tranid, created_date, customer_id, customer_name, hold_till, so_line, item_id, item_sku, item_name,\
  ...
  function writeCsv(ctx, rows, selectedSku) {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      var line = [
        ...
        r.item_name || "",
        Number(r.qty || 0),
        formatAgeFromPaidAt(r.line_paid_at),
        "",
      ];
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      out.push(
        "<tr>" +
          ...
          "</td>" +
          "<td>" +
          htmlEscape(formatAgeFromPaidAt(r.line_paid_at)) +
          "</td>" +
          '<td class="serial"></td>' +
          "</tr>",
      );
  ...
  function onRequest(ctx) {
    ...
    var rawRows = runReportRows();
    var split = splitRowsByHoldTill(rawRows);

    var allVisibleRows = split.visibleRows;
    var allSkippedHoldTillRows = split.skippedHoldTillRows;

    var rows = filterBySku(allVisibleRows, selectedSku);
    var skippedHoldTillFiltered = filterBySku(
      allSkippedHoldTillRows,
      selectedSku,
    );
    ...
    var skuOptions = buildSkuOptions(allVisibleRows);
    ...
    summary.defaultValue =
      ...
      "</b></div>" +
      "<div>Skipped due to hold till: <b>" +
      skippedHoldTillFiltered.length +
      "</b></div>" +
      "<div>Kit parents: <b>" +
      kitParents +
      "</b> &nbsp;|&nbsp; Kit components: <b>" +
    ...
    for (var k = 0; k < rows.length; k++) {
      ...
      sub.setSublistValue({
        id: "col_line_age",
        line: k,
        value: String(formatAgeFromPaidAt(rr.line_paid_at)),
      });
    }
