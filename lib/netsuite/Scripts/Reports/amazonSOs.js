  ...
  function runReport() {
    const soSearch = search.create({
      ...
      filters: [
        ["mainline", "is", "T"],
        "AND",
        ["formulatext: UPPER({salesrep})", "is", "AMAZON"],
      ],
      columns: [
    ...
    log.debug({ title: "Amazon SO count", details: results.length });
    return results;
  }
  ...
  function onRequest(context) {
    ...
    if (context.request.parameters.csv === "T") {
      ...
      context.response.addHeader({
        ...
        value: 'attachment; filename="amazon_sales_orders.csv"',
      });
